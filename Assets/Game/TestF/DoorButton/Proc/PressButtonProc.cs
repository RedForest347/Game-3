...
public class PressButtonProc : ProcessingBase, ICustomUpdate, ICustomAwake, ICustomDisable
{
    ...
    Group ButtonGroup = Group.Create(new ComponentsList<ButtonComp, ButtonAnimCmp>());
    ...
    void StartOpenDoor(int ent)
    {
        int door = Storage.GetComponent<ButtonAnimCmp>(ent).doorHolder.GetComponent<EntityBase>().entity;
    ...
    void PressButton()
    {
        if (Input.GetKeyDown(KeyCode.F))
        {
            ...
            if (button != 0)
            {
                if (InZone(Storage.GetComponent<ButtonComp>(button), EntityBase.GetEntity(PlayerGroup.GetEntitiesArray()[0]).transform.position))
                {
    ...
    bool InZone(ButtonComp door, Vector3 player_pos)
    {
        bool in_gorizontal;
